...
def get_data_from_year(year):
    ...
    return T_BACK_DEAL_HISTORY_df


def transform_silver(df):
    silver_df = (
        df.withColumn("partition_date", to_date("C_TRANSACTION_DATE", "yyyy-MM-dd"))
...
for data_year in range(2008, 2022):
    print(f"start load data from:  {data_year}")
    T_BACK_DEAL_HISTORY_df = get_data_from_year(data_year)
    silver_df = transform_silver(T_BACK_DEAL_HISTORY_df)
